...
- **M3 — The meta: started** — in-match **veterancy pips** (a kill grants a pip, max 3, +4% accuracy each, visible "veteran" factor in the % breakdown; in-match only, resets by construction). Next: archetype abilities + stat blocks, PP ranks + hidden MMR, accounts, stats pipeline
